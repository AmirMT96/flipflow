...
export default function StudyPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: deck }, { data: cards }, { data: reviews }] = await Promise.all([
        ...
        ids.length > 0 ? supabase.from('reviews').select('*').in('card_id', ids) : Promise.resolve({ data: [] }),
      ])
  ...
  const handleRate = useCallback(async (rating: Rating) => {
    ...
    else setTimeout(() => setCurrentIndex((i) => i + 1), 80)
  }, [queue, currentIndex, reviewMap, supabase])

  if (loading) {
    return <div className="min-h-screen flex items-center justify-center"><p className="text-gray-400 text-sm">Laden…</p></div>
  }
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-4 gap-4">
        ...
        <p className="text-gray-400 text-sm">
          Alle {queue.length} Karten aus <strong>{deckName}</strong> gelernt.
        </p>
        <Link
          href="/"
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="h-1 bg-gray-100">
        <div className="h-full bg-[#378ADD] transition-all" style={{ width: `${(currentIndex / queue.length) * 100}%` }} />
      </div>

      <div className="flex-1 flex flex-col items-center justify-center px-4 py-10">
        <div className="flip-card w-full max-w-lg cursor-pointer" style={{ height: 260 }} onClick={() => !flipped && setFlipped(true)}>
          <div className={`flip-card-inner ${flipped ? 'flipped' : ''}`}>
            <div className="flip-card-front bg-white border border-gray-100 rounded-2xl flex flex-col items-center justify-center p-8">
              ...
              {!flipped && <p className="mt-4 text-sm text-gray-400">Tippen zum Aufdecken</p>}
            </div>
            <div className="flip-card-back bg-white border border-gray-100 rounded-2xl flex flex-col items-center justify-center p-8">
              ...
              {card.example_sentence && (
                <p className="mt-3 text-sm text-gray-500 italic text-center max-w-xs">{card.example_sentence}</p>
              )}
            </div>
          </div>
        </div>

        <div className={`mt-8 grid grid-cols-4 gap-3 w-full max-w-lg transition-opacity duration-300 ${flipped ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
          {ratings.map(({ rating, label, color }) => (
            ...
            >
              ...
              <span className="text-xs mt-1 opacity-50">{getIntervalLabel(rating, smState)}</span>
            </button>
          ))}
        </div>
      </div>
