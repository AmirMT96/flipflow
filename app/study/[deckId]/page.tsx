...
import { computeNextReview, getIntervalLabel, type Rating, type SM2State } from '@/lib/sm2'
...
export default function StudyPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: deck }, { data: cards }, { data: reviews }] = await Promise.all([
        ...
        supabase.from('reviews').select('*').in(
          'card_id',
          (await supabase.from('cards').select('id').eq('deck_id', deckId)).data?.map((c: { id: string }) => c.id) ?? []
        ),
      ])
  ...
  if (done) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-4 gap-4">
        <Logo size="md" />
        <h2 className="text-2xl font-medium text-gray-900">Session complete!</h2>
        <p className="text-gray-400 text-sm">
          All {queue.length} cards reviewed for <strong>{deckName}</strong>.
        </p>
        <Link
          href="/"
          className="mt-4 px-5 py-2.5 bg-[#378ADD] text-white rounded-lg text-sm font-medium hover:bg-[#2d72c4] transition-colors"
        >
          Back to Dashboard / Zurück
        </Link>
      </div>
    )
  }
  ...
  const card = queue[currentIndex]
  const existing = reviewMap.get(card.id)
  const smState: SM2State = {
    ease_factor: existing?.ease_factor ?? 2.5,
    interval: existing?.interval ?? 1,
    repetitions: existing?.repetitions ?? 0,
  }

  const ratings: { rating: Rating; label: string; labelDe: string; color: string }[] = [
    { rating: 0, label: 'Again', labelDe: 'Nochmal', color: 'border-red-200 text-red-600 hover:bg-red-50' },
    { rating: 1, label: 'Hard', labelDe: 'Schwer', color: 'border-orange-200 text-orange-600 hover:bg-orange-50' },
    { rating: 2, label: 'Good', labelDe: 'Gut', color: 'border-[#378ADD]/30 text-[#378ADD] hover:bg-[#378ADD]/5' },
    { rating: 3, label: 'Easy', labelDe: 'Leicht', color: 'border-green-200 text-green-600 hover:bg-green-50' },
  ]
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
        ...
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-medium text-gray-700">
              {queue.length - currentIndex} card{queue.length - currentIndex !== 1 ? 's' : ''} left
            </span>
          </div>
        </div>
      </header>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-10">
        ...
        >
          {ratings.map(({ rating, label, labelDe, color }) => (
            <button
              key={rating}
            ...
            >
              ...
              <span className="text-xs opacity-70">{labelDe}</span>
              <span className="text-xs mt-1 opacity-50">
                {getIntervalLabel(rating, smState)}
              </span>
            </button>
          ))}
        </div>
