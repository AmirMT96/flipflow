...
export default async function StatsPage() {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const count = reviews?.filter((r) => r.last_reviewed?.split('T')[0] === iso).length ?? 0
    ...
    <>
      ...
      <main className="max-w-5xl mx-auto px-4 py-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
          <MiniStat label="Aktuelle Serie" value={`${streak}T`} />
          <MiniStat label="Längste Serie" value={`${longest}T`} />
          <MiniStat label="Reife Karten" value={`${pctMature}%`} accent />
          <MiniStat label="Karten gesamt" value={String(totalCards)} />
        </div>
...
function MiniStat({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="bg-white border border-gray-100 rounded-xl px-5 py-4">
      ...
      <p className={`text-2xl font-medium tabular-nums ${accent ? 'text-[#378ADD]' : 'text-gray-900'}`}>{value}</p>
    </div>
  )
}
